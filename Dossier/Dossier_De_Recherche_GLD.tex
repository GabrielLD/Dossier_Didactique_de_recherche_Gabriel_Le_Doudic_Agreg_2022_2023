...
\begin{figure}[!ht]
   \centering
   \resizebox{1\textwidth}{!}{\input{./figures/schema_injection_visualisation.pdf_tex}}
...
\noindent\begin{minipage}[c]{0.4\linewidth}
    \includegraphics[scale=.6]{./figures/Marangoni_flow_emulsion_Qa1p2mumolL_h7mm_h25mm.png}
    \captionof{figure}{ Deux photos superposés de l'écoulement de Marangoni. La première avec une épaisseur d'eau $h=7~\rm mm$ et un débit molaire $Q_{\rm a} = 1.2~\rm\upmu mol\cdot s^{-1}$ et la seconde avec $h=25~\rm mm$ et $Q_{\rm a} =1.2~\rm \upmu mol\cdot s^{-1}$.}
    \label{fig:EcoulementEpaisseurEau}
  \end{minipage}\hspace{.5cm}
  \begin{minipage}[c]{0.55\linewidth}
    \includegraphics[scale=.65]{./figures/lambda_dossierdidactique.png}
    \captionof{figure}{ (a) Dépendance de la distance moyenne $\lambda$ séparant deux pairs de tourbillons sur la couche d'eau d'épaisseur $h$ Les lignes en pointillés représentent une régression linéaire de la forme: $\lambda = Ah+\lambda_0$. (b) Dépendance de la distance moyenne $\lambda$ en fonction du débit molaire $Q_{\rm a}$ (la ligne en pointillé est un guide pour le regard).}
...
\begin{minipage}[c]{0.45\linewidth}
  \includegraphics[width=1\textwidth]{./figures/rolling_vortex_velocity_vorticity_v2.pdf}
  \captionof{figure}{(a,b) Mesure du champ de vitesse et du champ de vorticité pour $h=15~\rm mm$ et $Q_{\rm a}=1.2~\rm \upmu mol\cdot s^{-1}$. (a) Mesure du champ de vitesse. (b) Mesure du champ de vorticité ($\omega_y$).}\label{fig:PIV}
